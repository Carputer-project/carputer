...
void SensorManager::parseSensorJson(const QByteArray &data)
{
    ...
    // Wideband O2
    if (d.contains("wboAFR")) {
        double v = d.value("wboAFR").toDouble();
        if (qAbs(m_wboAFR - v) > 0.01) { m_wboAFR = v; emit wboAFRChanged(); }
    }
    if (d.contains("wboLambda")) {
        double v = d.value("wboLambda").toDouble();
        if (qAbs(m_wboLambda - v) > 0.01) { m_wboLambda = v; emit wboLambdaChanged(); }
    }
    if (d.contains("targetAFR")) {
        double v = d.value("targetAFR").toDouble();
        if (qAbs(m_targetAFR - v) > 0.01) { m_targetAFR = v; emit targetAFRChanged(); }
    }
    if (d.contains("fuelCorrection")) {
        double v = d.value("fuelCorrection").toDouble();
