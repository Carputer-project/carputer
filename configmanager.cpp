...
void ConfigManager::setGaugeBorderMode(const QString &v)
{
    if (m_gaugeBorderMode == v) return;
    m_gaugeBorderMode = v;
    m_settings.setValue("ui/gaugeBorderMode", v);
    m_settings.sync();
    emit gaugeBorderModeChanged();
}

void ConfigManager::setBackgroundImage(const QString &v)
{
    if (m_backgroundImage == v) return;
    m_backgroundImage = v;
    m_settings.setValue("ui/backgroundImage", v);
    m_settings.sync();
    emit backgroundImageChanged();
}

void ConfigManager::setBackgroundOpacity(double v)
{
    v = qBound(0.0, v, 1.0);
    if (qFuzzyCompare(m_backgroundOpacity, v)) return;
    m_backgroundOpacity = v;
    m_settings.setValue("ui/backgroundOpacity", v);
    m_settings.sync();
...
void ConfigManager::save()
{
    ...
    m_settings.setValue("ui/gaugeBorderMode",      m_gaugeBorderMode);
    m_settings.setValue("ui/backgroundImage",      m_backgroundImage);
    m_settings.setValue("ui/backgroundOpacity",    m_backgroundOpacity);
    m_settings.sync();
...
void ConfigManager::reload()
{
    ...
    m_backgroundImage  = m_settings.value("ui/backgroundImage", "").toString().trimmed();
    m_backgroundOpacity = qBound(0.0, m_settings.value("ui/backgroundOpacity", 0.3).toDouble(), 1.0);
    ...
    m_gaugeBorderMode = m_settings.value("ui/gaugeBorderMode", "gauge").toString();
