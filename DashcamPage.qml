...
Item {
            ...
            Repeater {
                ...
                Rectangle {
                    ...
                    Rectangle {
                        ...
                        radius: 4
                        color: "transparent"
                        border.width: dashcamPage.focusIndex === index ? 2 : 0
    ...
    Item {
        ...
        Column {
            ...
            Row {
                ...
                Rectangle {
                    ...
                    border.color: dvrManager.recording ? themeManager.statusRed : (dashcamPage.focusIndex === 2 ? themeManager.carBlue : themeManager.carBlue)
                    border.width: dashcamPage.focusIndex === 2 ? 3 : 2
                    Row {
                        anchors.centerIn: parent
                        spacing: 12
                        Text {
                ...
                Rectangle {
                    ...
                    border.color: dashcamPage.focusIndex === 3 ? themeManager.carBlue : themeManager.bgPanel
                    border.width: dashcamPage.focusIndex === 3 ? 3 : 1
                    Column {
                        anchors.centerIn: parent
                        spacing: 4
                        Text {
        ...
        Column {
            ...
            Rectangle {
                ...
                Rectangle {
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            width: 36; height: 28
                            radius: 4; color: dashcamPage.focusIndex === 3 ? themeManager.carBlue : themeManager.carBlueDim
                            Layout.alignment: Qt.AlignVCenter
                            border.width: dashcamPage.focusIndex === 3 ? 2 : 0
                            border.color: themeManager.carBlue
                            Text {
                                anchors.centerIn: parent
                        ...
                        Rectangle {
                            width: 36; height: 28
                            radius: 4; color: dashcamPage.focusIndex === 4 ? themeManager.carBlue : "#444"
                            border.width: dashcamPage.focusIndex === 4 ? 2 : 0
                            border.color: themeManager.carBlue
                            Layout.alignment: Qt.AlignVCenter
                            Text {
                                anchors.centerIn: parent
            ...
            Row {
                ...
                Rectangle {
                    width: 90; height: 34; radius: 6; color: dashcamPage.focusIndex === 5 ? Qt.rgba(themeManager.carBlue.r, themeManager.carBlue.g, themeManager.carBlue.b, 0.2) : themeManager.bgCard
                    border.width: dashcamPage.focusIndex === 5 ? 2 : 0
                    border.color: themeManager.carBlue
    ...
    Dialog {
        ...
        Keys.onPressed: {
            ...
            if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                var sources = ["/dev/video0", "/dev/video1", "/dev/video2", "udp://0.0.0.0:5600"]
                dvrManager.setCameraSource(sources[dialogFocus]); close()
                event.accepted = true; return
            }
            if (event.key === Qt.Key_Down && dialogFocus < 3) { dialogFocus++; event.accepted = true; return }
        ...
        Column {
            ...
            Repeater {
                ...
                Rectangle {
                    ...
                    color: cameraPicker.dialogFocus === index ? themeManager.carBlue : (dvrManager.cameraSource === modelData ? themeManager.carBlueDim : themeManager.bgCard)
                    Text {
                        anchors.centerIn: parent
                        text: modelData
    ...
    Dialog {
        ...
        Keys.onPressed: {
            ...
            if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                if (dialogFocus === 0) { close(); deleteConfirm.filePath = "" }
                else { dvrManager.deleteFile(deleteConfirm.filePath); close(); deleteConfirm.filePath = "" }
                event.accepted = true; return
            }
            if (event.key === Qt.Key_Escape) { close(); deleteConfirm.filePath = ""; event.accepted = true; return }
        }
        Column {
            width: parent.width
            spacing: 16
            ...
            Row {
                ...
                Rectangle {
                    ...
                    color: confirmDialog.dialogFocus === 0 ? themeManager.carBlue : themeManager.bgCard
                    Text { anchors.centerIn: parent; text: "Cancel"; color: confirmDialog.dialogFocus === 0 ? "#fff" : themeManager.textPrimary; font.pixelSize: 15 }
                    MouseArea { anchors.fill: parent; onClicked: { confirmDialog.close(); deleteConfirm.filePath = "" } }
                }
                Rectangle {
                    width: (parent.width - 10) / 2; height: 44; radius: 8
                    color: confirmDialog.dialogFocus === 1 ? themeManager.statusRed : "#330000"
                    border.color: themeManager.statusRed; border.width: confirmDialog.dialogFocus === 1 ? 3 : 1
                    Text { anchors.centerIn: parent; text: "Delete"; color: confirmDialog.dialogFocus === 1 ? "#fff" : themeManager.statusRed; font.pixelSize: 15; font.bold: true }
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
    ...
    function activateCurrentFocus() {
        if (focusIndex === 0) { activeTab = 0; dvrManager.startPreview(); return }
        if (focusIndex === 1) { activeTab = 1; return }
        if (focusIndex === 2 && activeTab === 0) { dvrManager.recording ? dvrManager.stopRecording() : dvrManager.startRecording(); return }
    ...
    function navigateRight() {
        if (activeTab === 0 && focusIndex < 3) { focusIndex++; return }
    ...
    function handleEscape() {
        ...
        if (confirmDialog.visible) { confirmDialog.close(); deleteConfirm.filePath = ""; return }
        root.activePage = 1
    }
    ...
    Keys.onPressed: {
        ...
        if (event.key === Qt.Key_Right) {
            if (activeTab === 0 && focusIndex < 3) { focusIndex++; event.accepted = true; return }
            if (activeTab === 1 && focusIndex < 5) { focusIndex++; event.accepted = true; return }
            event.accepted = true; return
        }
        if (event.key === Qt.Key_Down) {
            if (focusIndex <= 1 && activeTab === 0) { focusIndex = 2; event.accepted = true; return }
            if (focusIndex <= 1 && activeTab === 1) { focusIndex = 2; event.accepted = true; return }
            event.accepted = true; return
        }
        ...
        if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
            if (focusIndex === 0) { activeTab = 0; if (activeTab === 0) dvrManager.startPreview() }
            else if (focusIndex === 1) { activeTab = 1 }
            else if (focusIndex === 2 && activeTab === 0) { dvrManager.recording ? dvrManager.stopRecording() : dvrManager.startRecording() }
            else if (focusIndex === 3 && activeTab === 0) { cameraPicker.open() }
            else if (focusIndex === 2 && activeTab === 1) { } // seek slider - skip on Enter
        ...
        if (event.key === Qt.Key_Escape) { if (cameraPicker.visible) { cameraPicker.close(); event.accepted = true; return } if (confirmDialog.visible) { confirmDialog.close(); deleteConfirm.filePath = ""; event.accepted = true; return } }
    }
}
